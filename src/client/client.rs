...
fn main() {
    let num_ops = 10_000;  //TODO: Get this and keyrange from the settings file
    ...
    for n in 0..num_ops {
        if do_put_dist.sample(&mut thread_rng()) {
            ...
            let b = put(thread_rng().gen_range(0, key_high), String::from("AAA"));
            match b {
                true => put_success += 1,
                false => put_fail +=1,
            }
            ...
            let o = get(thread_rng().gen_range(0, key_high));
            match o {
...
fn put(key: KeyType, value: ValueType) -> bool {
    ...
    to_writer(&mut stream,&c)
        .expect("Unable write Put");
